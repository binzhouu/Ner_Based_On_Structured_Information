...
Date: 2019-09-25
"""
from model.bert_cnn_lstm_crf import BertCLC, BertCLCConfig
from read_data import BertData
import time
import random
import torch.optim as optim
from utils.functions import lr_decay, batchify_sequence_labeling_with_label, predict_check, evaluate, load_vocab
import torch
import logging
import sys
import numpy as np
	...
	for idx in range(config.epoch):
		...
		for batch_id in range(total_batch):
			...
			batch_word, batch_features, batch_wordlen, batch_wordrecover, batch_char, batch_charlen, batch_charrecover, \
				batch_label, mask = batchify_sequence_labeling_with_label(instance, config.gpu, if_train=True)
			...
			loss, tag_seq = model(
				batch_word, batch_features, batch_wordlen, batch_char, batch_charlen, batch_charrecover, mask, batch_char_mask, batch_label)
		...
		print("Epoch: %s, Loss: %s, Test: time: %.2fs, speed: %.2fst/s; acc: %.4f, p: %.4f, r: %.4f, f: %.4f" % (
