...
class CnnLstmAttnCrfConfig(object):
	...
	model_path = 'torch_model/ccnn_wlstm_attn_crf/best_model'


config = CnnLstmAttnCrfConfig()


class CnnLstmAttnCrf(nn.Module):
	def __init__(self, data):
		super(CnnLstmAttnCrf, self).__init__()
		...
		self.lstm = nn.LSTM(
			config.word_emb_dim + config.feature_emb_dim, config.hidden_dim // 2,
			num_layers=1, batch_first=True, bidirectional=True)
		self.droplstm = nn.Dropout(config.dropout)

		# attn层
		self.attention = ScaledDotProductAttention(temperature=np.power(config.d_k, 0.5))

		self.hidden2tag = nn.Linear(config.hidden_dim, data.label_alphabet_size + 2)

		self.crf = CRF(data.label_alphabet_size, config.gpu)

	def forward(self, batch_word, batch_features, batch_wordlen, batch_char, batch_charlen, batch_charrecover, mask, batch_label=None):
		...
		word_embs = torch.cat([word_embs, feat_embs], 2)
		...
		q, k, v = char_features, lstm_out, lstm_out
		attn_output, attn = self.attention(q, k, v)

		outputs = self.hidden2tag(attn_output)
