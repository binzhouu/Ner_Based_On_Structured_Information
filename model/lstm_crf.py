...
class BiLstmCrf(nn.Module):
	def __init__(self, data, configs):
		...
		if configs['random_embedding']:
			...
			pass
		self.lstm = nn.LSTM(
			configs['word_emb_dim'], configs['hidden_dim'] // 2, num_layers=configs['num_layers'],
	...
	def forward(self, batch_input, batch_len, batch_recover, mask, batch_label=None):
		...
		packed_words = pack_padded_sequence(word_embeds, batch_len.cpu().numpy(), batch_first=True)
		hidden = None
