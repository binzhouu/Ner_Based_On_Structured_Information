...
from model.char_bilstm_crf import CharBilstmCrf
from _read_data import Data
from utils.functions import batchify_sequence_labeling_charlevel, predict_check, evalute_sequence_labeling_charlevel
...
class Run(object):
	...
	@use_delete(True)
	def train(self, data):
		if self.encoder_type == 'char_bilstm_crf':
			model = CharBilstmCrf(data, self.configs)
		else:
			logger.info('No model is selected!')
			return
		optimizer = optim.SGD(
			model.parameters(), lr=self.configs['lr'], momentum=self.configs['momentum'],
		...
		for idx in range(self.configs['epoch']):
			...
			for batch_id in range(total_batch):
				...
				if not instance:
					continue
				_, _, _, _, batch_char, batch_charlen, batch_charrecover, batch_label, mask = \
					batchify_sequence_labeling_charlevel(instance, self.configs['gpu'], if_train=True)
				loss, tag_seq = model(batch_char, batch_charlen, batch_charrecover, mask, batch_label)
			...
			speed, acc, p, r, f, _, _ = evalute_sequence_labeling_charlevel(data, model, "dev", self.configs)
			...
			speed, acc, p, r, f, _, _ = evalute_sequence_labeling_charlevel(data, model, "test", self.configs)
			test_finish = time.time()
			test_cost = test_finish - dev_finish
			logger.info(
				"Epoch: %s, Loss: %s, Test: time: %.2fs, speed: %.2fst/s; acc: %.4f, p: %.4f, r: %.4f, f: %.4f" % (
